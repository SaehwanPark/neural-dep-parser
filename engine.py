...
def init_state(sentence_len: int, max_stack: int, max_buffer: int) -> ParserState:
  """
  initializes parser state for a sentence.
...
def extract_features(
  ...
  FIXED: safer indexing without jnp.maximum workaround that could access wrong positions
  """

  # helper to find children in the current dependency list
  def get_child(head_idx: int, side: int, rank: int) -> int:
  ...
  # 1. basic stack and buffer positions - FIXED: safe indexing
  s0 = state.stack[state.stack_ptr]

  # removed jnp.maximum workaround - jnp.where guards the access properly
  s1 = jnp.where(state.stack_ptr >= 1, state.stack[state.stack_ptr - 1], -1)
  s2 = jnp.where(state.stack_ptr >= 2, state.stack[state.stack_ptr - 2], -1)

  buf_size = state.buffer.shape[0]
  b0 = jnp.where(state.buffer_ptr < buf_size, state.buffer[state.buffer_ptr], -1)
  b1 = jnp.where(
    state.buffer_ptr + 1 < buf_size, state.buffer[state.buffer_ptr + 1], -1
  )
  b2 = jnp.where(
    state.buffer_ptr + 2 < buf_size, state.buffer[state.buffer_ptr + 2], -1
  )
  ...
  # FIXED: clamp indices before access, then mask with where
  safe_indices = jnp.maximum(pos_indices, 0)
  word_features = jnp.where(
    pos_indices >= 0, sentence.words[safe_indices], config.NULL_ID
  )
  pos_features = jnp.where(
    pos_indices >= 0, sentence.pos[safe_indices], config.P_NULL_ID
  )
